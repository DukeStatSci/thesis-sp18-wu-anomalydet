...
\subsection{Constructing a Low Rank
Matrix}\label{constructing-a-low-rank-matrix}

\subsection{Approximating Low Rank with Leave One Out Cross
Validation}\label{approximating-low-rank-with-leave-one-out-cross-validation}
...
\(Y^{(1)}, Y^{(2)}, Y^{(3)}, Y^{(4)} \in \mathbb{R}_{m \times n}\). The
previous technique fails to take into account the differing sample size
and variance in each cell (source port \(i\), destination port \(j\)),
so the algorithm treated each \(y_{ij}^{k}\), which represents the mean
of all observations in cell \((i,j)\) for continuous feature \(k\), as a
single value with no consideration of differing sample sizes and
variances between cells.
...
\(w_i = \frac{\tau_j}{\sqrt(n_{ij}}\), and \(\beta^T = u_i\).

Using this model, we construct a Gibbs Sampler:
\[\tilde{y} = X\beta + \sigma W \epsilon\] where \(W\) is the diagonal
matrix of weights. This is a specific form of the Generalized Least
Squares Model.

In this case:
\[\Sigma^{-1} = (\sigma^2W^{-1})^{-1} = \frac{W}{\sigma^2}\]
...
\[ \begin{cases}
    ...
    \end{cases}\]
\[SSR_W = (y - X\beta)^TW(y-X\beta)\] Priors:
...
parameter in the statistical model, which iteratively creates an
