...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{### MATRIX VISUALIZATIONS}
\CommentTok{# https://stackoverflow.com/questions/5453336/plot-correlation-matrix-into-a-graph}
\CommentTok{# levelplot(means, xlab = "Source Ports", ylab = "Destination Ports", }
\CommentTok{#           main = "Means of SrcBytes By Ports", }
\CommentTok{#           col.regions=rgb.palette(120))}
\KeywordTok{levelplot}\NormalTok{(freqs, }\DataTypeTok{xlab =} \StringTok{"Source Ports"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"Destination Ports"}\NormalTok{,  }
          \DataTypeTok{col.regions =} \KeywordTok{heat.colors}\NormalTok{(}\DecValTok{16}\NormalTok{)[}\KeywordTok{length}\NormalTok{(}\KeywordTok{heat.colors}\NormalTok{(}\DecValTok{16}\NormalTok{)):}\DecValTok{1}\NormalTok{],}
          \DataTypeTok{main =} \StringTok{"Sample Sizes of Port Combinations"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
