...
\includegraphics{thesis_files/figure-latex/unnamed-chunk-2-1.pdf} The
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p<-}\DecValTok{5}
\NormalTok{beta<-}\KeywordTok{rnorm}\NormalTok{(p)}
\NormalTok{X =}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rnorm}\NormalTok{(m *}\StringTok{ }\NormalTok{p, }\DataTypeTok{mean=}\DecValTok{0}\NormalTok{, }\DataTypeTok{sd=}\DecValTok{1}\NormalTok{), m, p)}
\NormalTok{W =}\StringTok{ }\KeywordTok{diag}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{rexp}\NormalTok{(m), }\DataTypeTok{nrow =} \NormalTok{m, }\DataTypeTok{ncol =} \NormalTok{m)}
\NormalTok{y =}\StringTok{ }\NormalTok{X%*%beta  +}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{rnorm}\NormalTok{(m *}\StringTok{ }\DecValTok{1}\NormalTok{, }\DataTypeTok{mean=}\DecValTok{0}\NormalTok{, }\DataTypeTok{sd=}\DecValTok{1}\NormalTok{), m, }\DecValTok{1}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(beta, }\KeywordTok{lm}\NormalTok{(y~}\StringTok{ }\NormalTok{-}\DecValTok{1}\NormalTok{+}\StringTok{ }\NormalTok{X)$coef)}
