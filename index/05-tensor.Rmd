...
Vectorizing the inputs in the likelihood yields: $$\sum_j\sum_k[\bar t_{ijk} - a_i^T(b_i \cdotp c_k)]\frac{n_{ijk}}{\sigma^2_{ijk}}$$ where $a_i \in \mathbb{R_{m \times r}}$, $b_j \in \mathbb{R_{n \times r}}$, and $c_k \in \mathbb{R_{4 \times r}}$. Summing across $j$ and $k$ in this case solves for the $ith$ row slice of the tensor.
