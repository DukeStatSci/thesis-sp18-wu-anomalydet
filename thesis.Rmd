...
```{r}
#argus = read.csv("argus-anon-20170201.csv")
argus = readRDS("Dataset/argus_full.rds")
#argus = readRDS("Dataset/argus_subset.rds") #subset of 50000 values
summary(argus)
```
...
#Singulvar Value Decomposition

#Create Mean and Frequency Matrix
```{r}
#create matrix 
uniq_Sport = sort(unique(argus$Sport))
uniq_Dport = sort(unique(argus$Dport))
n_Sport = length(uniq_Sport)
n_Dport = length(uniq_Dport)

ports_mean_matrix = matrix(NA, nrow = n_Sport, ncol = n_Dport)
dimnames(ports_mean_matrix) = list(uniq_Sport, uniq_Dport)

ports_freq_matrix = matrix(0, nrow = n_Sport, ncol = n_Dport)
dimnames(ports_freq_matrix) = list(uniq_Sport, uniq_Dport)
  
update_mean = function(prev_mean, obs, freq){
  return ((prev_mean * freq + obs) / (freq + 1))
}

for (s in 1:n_Sport){
  for (d in 1:n_Dport){
    combination = argus[is.element(argus$Sport, uniq_Sport[s]) 
                        & is.element(argus$Dport, uniq_Dport[d]),]
    obs = combination$SrcBytes
    n_obs = length(obs)
    if (n_obs > 0){
      for (i in 1:n_obs){
        freq = ports_freq_matrix[s,d] 
        if (freq == 0){
          ports_mean_matrix[s,d] = obs[i]
        }
        else{
          prev_mean = ports_mean_matrix[s,d]
          ports_mean_matrix[s,d] = update_mean(prev_mean, obs[i], freq)
        }
...
head(ports_mean_matrix)
head(ports_freq_matrix)
```

#Value Imputation 
```{r}
#softImpute SVD
fit = softImpute(ports_mean_matrix,rank.max=5,lambda=3,trace=TRUE)
...
There are $m$ source ports and $n$ destination ports. $Y \in {\rm I\!R}^{m x n}$, is the matrix that stores the means of the combinations of source ports and destination ports. $Y$ has a lot of missingness because not every source port interacts with every destination port. $F \in {\rm I\!R}^{m x n}$ is a sparse matrix that represents the frequencies of combinations, i.e $F[32242,12312]$ represents the number of observations for the 32242 12312 port interaction. $M \in {\rm I\!R}^{m x n}$ represents a boolean matrix of whether the corresponding $Y$ values are missing. $Y[M]$ represents all of the missing values of $Y$.
...
The repeated imputation procedure solves $Y^{(s)} = R_k(Y^{(s-1)})[M]$ where $R_k$ is the best rank-k approximation for the $s$-th step. For each step $(s)$ use singular value decomposition to decompose $$Y^{(s)} =  U^{(s)}DV^{T(s)}$$ where $D$ is a diagonal matrix of the singular values, $U$ is the left singular vectors of $Y$ and $V$ is the right singular vectors of $Y$. 

The Eckart-Young-Mirsky (EYM) Theorem provides the best rank-k approximation for the missing values in $Y^{(s+1)}$. Recall $Y[M]$ represents all of the missing values of $Y$. Applying the EYM theorem:  $$Y^{(s+1)}[M] = (U[,1:k]^{(s)}DV[,1:k]^{T(s)})[M]$$
Where $U[,1:k]$ represents the first $k$ columns of $U$. 

The EYM rank approximation is repeated until the relative difference between $Y^{(s+1)}$ and $Y^{(s)}$ falls below a set threshold, $T$. The relative difference threshold is expressed: $$\|Y^{(s+1)}-Y^{(s)}\|_2 < T$$ Frobenius norm? L2 norm? 
